...
- `/ws/chat` is now v1-only: legacy `type: "method"` client frames are rejected and the server no longer emits legacy startup, event, result, or error frames.
...
- `/ws/chat` 现在接受 v1 `initialize`、`turn/start`、`turn/cancel`、`approval/respond`、`tool/respond` 和 `user_input/respond` JSON-RPC 形态帧
