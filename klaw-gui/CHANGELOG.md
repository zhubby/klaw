...
- 顶部菜单栏的菜单标题与下拉菜单项现在显示对应图标，并在下拉菜单的相邻菜单项之间加入分割线。
...
- System 面板的主机信息、程序磁盘使用与环境视图改用带图标的固定 `egui_dock` 标签页切换。
- System 面板的程序磁盘使用视图现在左右两列各自独立滚动，目录列表和饼图图例互不牵动。
- Voice 配置弹窗的通用与 provider 配置页改用带图标的固定 `egui_dock` 标签页切换。
- Gateway 面板的网关状态与 Tailscale 区域改用带图标的固定 `egui_dock` 标签页组织。
- Profile Prompt 面板的角色提示词文档与系统提示词预览改用带图标的固定 `egui_dock` 标签页切换。
- Voice 配置弹窗现在限制最大高度，避免撑满整个 GUI 内容区。
...
- System 面板主机信息中的系统信息表格现在按可见宽度二等分左右两列，避免左右列分布不均。
...
- ACP 面板全面支持多语言（代理表格、统计区、配置表单、全局设置、删除确认、详情窗口、测试提示窗口、内容块渲染、权限标签、通知消息等均随语言设置切换）
- LLM 面板全面支持多语言（筛选栏、表格列标题、上下文菜单、审计详情窗口、排序标签、状态显示、通知消息等均随语言设置切换）
- MCP 面板全面支持多语言（服务器表格、配置表单、全局设置、详情窗口、上下文菜单、状态标签、Markdown 详情、通知消息等均随语言设置切换）
...
- `Tools` 面板现在会列出并允许编辑 `knowledge` 工具开关与检索参数
...
- GUI 启动时现在会在后台异步检查 GitHub 最新 release，并基于 `release-vX.Y.Z` 名称做语义化版本比对；检测到新版本时，底部状态栏版本位置会显示可点击的更新提示图标

### Fixed

- `Gateway` 面板主区域现在支持垂直滚动；当窗口高度不足时，底部的 Tailscale 区域和公网暴露警告不再被截断而无法访问
...
- `Cron` 面板的 `Run Now` 改为后台 request-handle 轮询模式，不再在按钮点击时同步阻塞 egui 线程等待 runtime 响应
...
- `Analyze Dashboard` 的趋势图现在默认使用平滑曲线渲染，保留原有图例、tooltip 与时间轴格式

### Fixed

- `Configuration` 面板标题下方的操作区现在带图标展示，`Reset` 使用橙色强调，同时 TOML 编辑器新增搜索定位与上一处/下一处跳转
- `Configuration` 面板 TOML 搜索现在会把所有命中直接高亮在编辑器中，并在按下回车或点击上一处/下一处后把视口实际滚动到命中的文本位置；回车首次定位当前命中，后续继续跳下一条，`Shift+Enter` 则回到上一条
