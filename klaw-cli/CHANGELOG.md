...
- `/approve <approval_id>` 在 shell 执行失败后不再强制进入“只总结、不准调工具”的 follow-up；现在会保留 channel metadata，并把后续 turn 限制为最多一次额外工具调用，允许模型对明显可修复的命令/参数错误自动重试一次
...
- `voice` tool 的注册重新与 `voice.enabled` 绑定；runtime/tool sync 现在要求 `voice.enabled=true` 且 `tools.voice.enabled=true` 才会暴露该工具
...
- runtime 现在会异步持久化每次 tool 调用的结构化 `tool_audit` 记录，并把子代理工具调用一并回写到父 session 供 GUI 诊断
